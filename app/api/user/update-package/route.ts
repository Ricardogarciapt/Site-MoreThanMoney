...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate packageId if necessary

    const updatedUser = await db.updateUser(userId, { package: packageId })

    if (!updatedUser) {
      return NextResponse.json({ error: "Failed to update user package or user not found" }, { status: 404 })
    }
    ...
    console.error("Error in /api/user/update-package:", error)
