...
export default function CoursesManagerClient() {
  ...
  const removeBootcampVideo = (index: number) => {
    const newVideos = [...bootcampVideos]
    newVideos.splice(index, 1)
    setBootcampVideos(newVideos)
  }

  const updateBootcampVideo = (index: number, field: string, value: string) => {
    const newVideos = [...bootcampVideos]
  ...
  const removeDistributorVideo = (index: number) => {
    const newVideos = [...distributorVideos]
    newVideos.splice(index, 1)
    setDistributorVideos(newVideos)
  }

  const updateDistributorVideo = (index: number, field: string, value: string) => {
    const newVideos = [...distributorVideos]
    newVideos[index][field] = value
    setDistributorVideos(newVideos)
  }
